...
export default function AgentRunPanel({
  ...
  const isContractViolation = state.error?.code === 'CONTRACT_VIOLATION';
    ...
    <div className="w-full rounded-xl border border-gray-100 bg-white p-5 space-y-4">
        ...
        <section>
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
            ...
            <p className="mt-2 text-xs text-amber-700">
              Phase 1 では対話応答は未対応です。指示に情報を追加して再実行してください。
            </p>
          </div>
        </section>
      )}
      ...
      {state.final && (
        <section>
          ...
          <div className="rounded-lg border border-gray-100 bg-white p-4 prose prose-sm max-w-none">
