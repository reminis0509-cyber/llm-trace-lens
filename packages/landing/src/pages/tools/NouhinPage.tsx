/**
 * /tools/nouhin — 納品書テンプレート 無料 (Freemium 第 1 層 Phase B)
 *
 * 戦略 doc Section 5.6 / Section 18.2.N (Founder 承認 2026-04-29)
 *
 * 役割:
 *   - SEO 集客装置。ターゲット流入語「納品書 テンプレート 無料」(704/月) ほか
...
function trackEvent(
  name:
    | 'tools_nouhin_page_view'
    | 'tools_nouhin_form_started'
    | 'tools_nouhin_pdf_downloaded'
    | 'tools_nouhin_cta_clicked',
...
export default function NouhinPage() {
  ...
  const [meta, setMeta] = useState<DeliveryForm>(() => {
    const today = todayIso();
    return {
      deliveryNumber: `DLV-${today.replace(/-/g, '')}-001`,
      issueDate: today,
      deliveryDate: today,
      subject: '',
      notes: '',
  ...
  useEffect(() => {
    trackEvent('tools_nouhin_page_view');
  }, []);

  function markFormStarted(): void {
    if (formStartedRef.current) return;
    formStartedRef.current = true;
    trackEvent('tools_nouhin_form_started');
  }
  ...
  async function handleDownloadPdf(): Promise<void> {
    ...
    try {
      ...
      const pdfData: DeliveryNotePdfData = {
        delivery_number: meta.deliveryNumber || undefined,
      ...
      const blob = await generateDeliveryNotePdf(pdfData, issuerInfo);
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      const safeNumber = (meta.deliveryNumber || 'delivery').replace(/[^a-zA-Z0-9_-]/g, '_');
      ...
      trackEvent('tools_nouhin_pdf_downloaded', {
  ...
  function handleCtaClick(target: 'signup' | 'tools'): void {
    trackEvent('tools_nouhin_cta_clicked', { target });
  }
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            <div className="bg-white rounded-xl border border-slate-200 p-5 sm:p-7 space-y-7">
              ...
              <Fieldset legend="納品書情報">
                <TextField label="納品番号" value={meta.deliveryNumber} onChange={(v) => { markFormStarted(); setMeta((s) => ({ ...s, deliveryNumber: v })); }} placeholder="DLV-20260429-001" />
                <TextField label="件名" value={meta.subject} onChange={(v) => { markFormStarted(); setMeta((s) => ({ ...s, subject: v })); }} placeholder="○○商品の納品" />
                <DateField label="発行日" value={meta.issueDate} onChange={(v) => { markFormStarted(); setMeta((s) => ({ ...s, issueDate: v })); }} />
                <DateField label="納品日" value={meta.deliveryDate} onChange={(v) => { markFormStarted(); setMeta((s) => ({ ...s, deliveryDate: v })); }} />
              </Fieldset>
                ...
                <div className="grid grid-cols-2 gap-4 pb-4 mb-4 border-b border-slate-200">
                  <div>
                    ...
                    <div className="mt-3 space-y-0.5 text-xs text-slate-600">
                      <p>納品番号: {meta.deliveryNumber || '---'}</p>
                      <p>発行日: {formatJapaneseDate(meta.issueDate) || '---'}</p>
                      <p>納品日: {formatJapaneseDate(meta.deliveryDate) || '---'}</p>
                    </div>
                  </div>
