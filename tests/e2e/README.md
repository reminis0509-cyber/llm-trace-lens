...
AI Employee v1 (2026-04-20 着地) の回帰防止を目的とした E2E harness。
Landing / Dashboard の UI 表面のみをカバーし、backend server と DB は
起動しない。Supabase / backend API は `tests/e2e/fixtures/dashboard.ts`
で mock される。
...
├── connector-settings.spec.ts     <- /dashboard/settings/connectors
├── nav.spec.ts                    <- Dashboard 7 タブ遷移
...
これらは v2 (Full Manus TTP) 着地後、qa-engineer が本 README に追記する。
