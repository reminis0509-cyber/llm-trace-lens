...
FujiTraceは、複雑な設定や長期的な統合を必要とせず、**即座に価値を提供する**ことを最優先に設計されています。
...
FujiTraceは、プロバイダに依存しない設計を採用しています。
...
fetch('http://localhost:3000/v1/chat/completions', {
  ...
  body: JSON.stringify({
    provider: 'openai',  // または 'anthropic', 'gemini'
    model: 'gpt-4',
