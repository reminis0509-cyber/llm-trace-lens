...
自分が開発している [FujiTrace](https://www.fujitrace.jp) では、RAGトレースの自動記録、LLM-as-Judgeによる品質スコアリング、日本語PII検出（15パターン以上）をプロキシ型で提供している。baseURLを変えるだけで導入できるので、既存のRAGアプリにもすぐ組み込める。

技術者向けの詳細は[こちら](https://www.fujitrace.jp/for-engineers)。

---

質問やフィードバックがあればコメントか [X(@FujiTrace)](https://x.com/FujiTrace) で気軽にどうぞ。
