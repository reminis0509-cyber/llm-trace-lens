...
**最終更新**: 2026年2月22日
**バージョン**: 0.5.2
**ステータス**: MVP完成 + SaaS化完了 + エンタープライズ機能追加 + ストレージ最適化 + LLM-as-Judge評価 + Vercel本番デプロイ完了 + **RBAC・チーム招待・リアルタイム機能追加**
...
| **PostgreSQLデフォルト化** | ✅ | 本番推奨ストレージをPostgreSQLに変更 |
| **KV保持ポリシー** | ✅ | 自動削除・使用量監視・警告表示 |
| **RBAC (ロールベースアクセス制御)** | ✅ NEW | owner/admin/member ロール階層 |
| **チーム招待機能** | ✅ NEW | 招待リンク・メール招待・有効期限管理 |
| **リアルタイムトレース更新** | ✅ NEW | Supabase Realtime WebSocket統合 |
...
| **LLM-as-Judge評価** | ✅ | Faithfulness・Answer Relevance自動評価 |
| **Membersページ** | ✅ NEW | メンバー一覧・招待・ロール変更UI |
| **リアルタイム接続状態** | ✅ NEW | トレース一覧に接続状態インジケーター表示 |
...
## 3. v0.5.2 RBAC・チーム招待・リアルタイム機能（2026年2月22日）

### 3.1 ロールベースアクセス制御（RBAC）

ワークスペース内のユーザーに対して、役割に基づいたアクセス制御を実装しました。

**ロール階層:**
| ロール | 権限レベル | 説明 |
|--------|-----------|------|
| owner | 3 | 全権限 + ワークスペース削除 |
| admin | 2 | メンバー管理 + 設定変更 |
| member | 1 | 閲覧 + トレース送信のみ |

**実装内容:**
- **RBACミドルウェア**: `requireRole(minimumRole)` でエンドポイント保護
- **ユーザー情報抽出**: `X-User-ID`, `X-User-Email` ヘッダーからユーザー特定
- **ロール階層比較**: owner(3) > admin(2) > member(1)

**新規ファイル:**
- `src/types/rbac.ts` - RBAC型定義
- `src/middleware/rbac.ts` - RBACミドルウェア
- `src/routes/members.ts` - メンバー管理API

### 3.2 チーム招待機能

ワークスペースへのメンバー招待機能を実装しました。
...
**マイグレーション:**
- `migrations/006_add_invitations.ts` - 招待テーブル作成
...
{ "token": "invite_xxx", "email": "newuser@example.com" }
...
Supabase Realtimeを使用したWebSocket統合を実装しました。
...
- Supabase未設定時は自動フォールバック（ポーリング継続）
- ワークスペース単位のチャネル分離

**新規ファイル:**
- `src/lib/realtime.ts` - Supabase Realtime ブロードキャスト
- `packages/dashboard/src/hooks/useRealtimeTraces.ts` - リアルタイム購読フック

**環境変数:**
```bash
# Supabase Realtime（オプション）
...
**新規コンポーネント:**
- `packages/dashboard/src/pages/Members.tsx` - メンバー管理ページ
- `packages/dashboard/src/pages/InviteAccept.tsx` - 招待承諾ページ
- `packages/dashboard/src/contexts/RoleContext.tsx` - ロール管理コンテキスト
- `packages/dashboard/src/components/ErrorBoundary.tsx` - エラーバウンダリ

**変更ファイル:**
- `packages/dashboard/src/pages/Dashboard.tsx` - Membersタブ追加
- `packages/dashboard/src/components/TraceList.tsx` - リアルタイム接続状態表示
- `packages/dashboard/src/api/client.ts` - membersApi追加

---

## 4. v0.5.1 Vercel本番デプロイ（2026年2月21日）

### 4.0 本番環境デプロイ完了

Vercelへの本番デプロイを完了しました。

**本番URL**: https://llm-trace-lens1.vercel.app

**デプロイ構成:**
- **フレームワーク**: Vite (React SPA)
- **ビルド**: `cd packages/dashboard && npm run build`
- **出力**: `packages/dashboard/dist`
...
```json
{
  ...
  "rewrites": [
    ...
    { "source": "/v1/(.*)", "destination": "/api/index.ts" },
...
```bash
# Supabase認証
VITE_SUPABASE_URL=https://qbpmfereuhnpuvnjraxt.supabase.co
...
**解決した問題:**
1. **Vercel本番ビルドでdevDependencies未インストール**: `vite`, `@vitejs/plugin-react`等をdependenciesに移動
2. **TypeScript型エラー**: ダッシュボードのみビルドするよう変更（ルートtsc除外）
3. **Supabase URL/Key**: VITE_プレフィックス付きで設定

**デプロイコミット**: `e841885`

---

## 5. v0.5.0 新機能（2026年2月18日）

### 5.0 LLM-as-Judge評価エンジン（Phase 1 MVP）
...
## 6. v0.4.1 新機能（2026年2月16日）

### 6.0 PostgreSQLデフォルト化 + KV保持ポリシー（Priority S）
...
## 7. v0.4.0 新機能（2026年2月15日）

### 7.1 閾値のブラックボックス化（Priority S）
...
### 7.4 Gemini/DeepSeek ストリーミング対応（既存）
...
| 4 | Slack/Teams連携 | ✅ 完了 |
| 5 | LLM-as-Judge評価 | ✅ 完了 |
| 5 | ロールベースアクセス制御（RBAC） | ✅ 完了 |
| 5 | チーム招待機能 | ✅ 完了 |
| 5 | ダッシュボードリアルタイム更新 | ✅ 完了 |
...
**最終更新**: 2026年2月22日（v0.5.2 RBAC・チーム招待・リアルタイム機能追加）
