...
**最終更新**: 2026年2月15日
**バージョン**: 0.4.0
**ステータス**: MVP完成 + SaaS化完了 + エンタープライズ機能追加
...
| **閾値ブラックボックス化** | ✅ NEW | リスクスコアリング・Admin API |
| **フィードバック機能** | ✅ NEW | 誤検知フィードバック・パターン分析 |
| **Slack/Teams連携** | ✅ NEW | Webhook通知・Block Kit/Adaptive Cards |
...
| **Analyticsページ** | ✅ NEW | フィードバック統計・パターン分析 |
| **Integrationsページ** | ✅ NEW | Slack/Teams Webhook設定UI |
...
## 3. v0.4.0 新機能（2026年2月15日）

### 3.1 閾値のブラックボックス化（Priority S）
...
### 3.4 Gemini/DeepSeek ストリーミング対応（既存）
...
```
 ...
 ✓ src/tests/validation/scoring.test.ts (13 tests) [NEW]
 ...
 ✓ dist/src/tests/validation/confidence.test.js (5 tests)
 ✓ dist/src/tests/validation/risk.test.js (6 tests)
 ✓ dist/src/tests/validation/japanese-pii.test.js (16 tests)
...
4. **トレース保持制限**: KVでは直近1000件/30日間のみ保持
...
**最終更新**: 2026年2月15日（v0.4.0 閾値ブラックボックス化・フィードバック機能・Slack/Teams連携追加）
