...
export class OpenAIEnforcer {
  ...
  async enforce(request: LLMRequest): Promise<StructuredResponse> {
    ...
    // Ensure answer is always a string (LLM may return object for structured content like 見積書)
    ...
    // Ensure each alternative is a string
    structured.alternatives = structured.alternatives.map((a: unknown) =>
