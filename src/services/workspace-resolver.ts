...
// キャッシュ設定
/** デフォルトのキャッシュ有効期限: 300秒（5分） */
const DEFAULT_CACHE_TTL_SECONDS = 300;
const DEFAULT_CACHE_TTL = parseInt(process.env.API_KEY_CACHE_TTL || String(DEFAULT_CACHE_TTL_SECONDS), 10);
const CACHE_ENABLED = process.env.ENABLE_API_KEY_CACHE !== 'false';

/** キャッシュクリーンアップ間隔: 5分（ミリ秒） */
const CACHE_CLEANUP_INTERVAL_MS = 5 * 60 * 1000;

/** アクセスログの保持期間: 30日間（秒） */
const ACCESS_LOG_EXPIRY_SECONDS = 60 * 60 * 24 * 30;
...
/** キャッシュの最大エントリ数（LRU制限） */
const MAX_CACHE_ENTRIES = parseInt(process.env.API_KEY_CACHE_MAX_ENTRIES || '1000', 10);
...
class ApiKeyCache {
  ...
  constructor(ttlSeconds: number = DEFAULT_CACHE_TTL, maxEntries: number = MAX_CACHE_ENTRIES) {
  ...
  get(apiKeyHash: string): WorkspaceInfo | undefined {
    ...
    // LRU: アクセス時に末尾に移動（Map は挿入順を保持）
    this.cache.delete(apiKeyHash);
    this.cache.set(apiKeyHash, entry);

    return entry.data;
  }

  set(apiKeyHash: string, data: WorkspaceInfo): void {
    // 既存エントリがあれば削除（位置更新のため）
    if (this.cache.has(apiKeyHash)) {
      this.cache.delete(apiKeyHash);
    }

    // LRU制限: 最大数を超えたら最古のエントリを削除
...
export async function registerApiKeyMapping(params: {
  ...
  const {
    ...
    customerName,
    provider,
    expiryDays = parseInt(process.env.API_KEY_EXPIRY_DAYS || String(DEFAULT_API_KEY_EXPIRY_DAYS), 10),
  } = params;
...
export async function logApiKeyAccess(params: {
  ...
  const log: ApiKeyAccessLog = {
    id: `access_${Date.now()}_${randomBytes(8).toString('hex')}`,
    apiKeyHash,
  ...
  try {
    // ログを保存
    await kv.set(`${API_KEY_ACCESS_LOG_PREFIX}${log.id}`, log, { ex: ACCESS_LOG_EXPIRY_SECONDS });
 ...
 * 期限切れ間近のAPIキーを取得
 */
export async function getExpiringApiKeys(daysBeforeExpiry: number = DEFAULT_EXPIRY_WARNING_DAYS): Promise<ApiKeyMapping[]> {
  if (!isKVAvailable()) return [];
