...
- **自社 vs 宛先の厳格な区別**: `company_info` はユーザー自身の会社(=書類の送り手/自社)。見積書・請求書等の「宛先」(=相手先/顧客)に `company_info` を使うのは禁止。宛先はユーザーの最新メッセージか直近の依頼文から抽出する。会話履歴に「会社情報を登録します」ターンが含まれる場合、その会社名は自社情報であって宛先ではない。
