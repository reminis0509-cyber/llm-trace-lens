...
export interface WebhookConfig {
  ...
  retries?: number;   // 最大リトライ回数（デフォルト: 3）
  timeout?: number;   // タイムアウト（ミリ秒、デフォルト: 5000ms = 5秒）
}

// Webhookのデフォルト設定
const DEFAULT_WEBHOOK_TIMEOUT_MS = 5000;  // 5秒
const DEFAULT_MAX_RETRIES = 3;            // 最大3回リトライ
  ...
  constructor(config?: WebhookConfig) {
    if (config) {
      ...
      if (!urlCheck.safe) {
        ...
        this.enabled = false;
        this.url = '';
        this.timeout = DEFAULT_WEBHOOK_TIMEOUT_MS;
      ...
      this.enabled = true;
      this.url = config.url;
      this.timeout = config.timeout || DEFAULT_WEBHOOK_TIMEOUT_MS;
      ...
      this.timeout = parseInt(process.env.WEBHOOK_TIMEOUT_MS || String(DEFAULT_WEBHOOK_TIMEOUT_MS));
      this.maxRetries = parseInt(process.env.WEBHOOK_MAX_RETRIES || String(DEFAULT_MAX_RETRIES));
