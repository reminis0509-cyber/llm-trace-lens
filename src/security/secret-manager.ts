...
/** 初期化ベクトル(IV)の長さ: 16バイト (128ビット、GCMの推奨値) */
const IV_LENGTH = 16;

/** 認証タグの長さ: 16バイト (128ビット、GCMの最大値) */
const TAG_LENGTH = 16;

/** 暗号化キーの長さ: 32バイト (256ビット、AES-256用) */
const KEY_LENGTH = 32;

/** アクセスログの保持期間: 1年間（秒） */
const ACCESS_LOG_EXPIRY_SECONDS = 60 * 60 * 24 * 365;

/** デフォルトのシークレットローテーション間隔（日） */
...
function getMasterKey(): Buffer {
  ...
  if (!masterKeyEnv) {
    if (process.env.NODE_ENV === 'production') {
      throw new Error('[SecretManager] SECRET_ENCRYPTION_KEY は本番環境では必須です。base64エンコードされた32バイトのキーを設定してください。');
    }
    // 開発環境のみ: インスタンス固有のフォールバックキーを生成（推測困難にする）
    console.warn('[SecretManager] SECRET_ENCRYPTION_KEY 未設定。開発用フォールバックキーを使用中（本番では使用しないでください）');
    return createHash('sha256').update(`dev-fallback-${process.cwd()}-${process.pid}`).digest();
  }

  // Base64エンコードされた32バイト(256ビット)のキーを期待
  const key = Buffer.from(masterKeyEnv, 'base64');
  if (key.length !== KEY_LENGTH) {
    throw new Error(`SECRET_ENCRYPTION_KEY must be a ${KEY_LENGTH}-byte key encoded in base64`);
  }
...
async function logAccess(params: LogAccessParams): Promise<void> {
  ...
  try {
    ...
    await kv.set(logKey, log, { ex: ACCESS_LOG_EXPIRY_SECONDS });
...
export async function getSecretsNeedingRotation(
  daysBeforeExpiry: number = DEFAULT_NOTIFY_BEFORE_DAYS
