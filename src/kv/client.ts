...
/** トレースの有効期限: 7日間（秒） */
const TRACE_EXPIRY_SECONDS = 60 * 60 * 24 * 7;

/** ワークスペーストレースの有効期限: 30日間（秒） */
const WORKSPACE_TRACE_EXPIRY_SECONDS = 60 * 60 * 24 * 30;

/** グローバルトレースの最大保存数 */
const MAX_GLOBAL_TRACES = 100;

/** ワークスペースごとの最大トレース数 */
const MAX_WORKSPACE_TRACES = 1000;

/** フィードバックの有効期限: 90日間（秒） */
const FEEDBACK_EXPIRY_SECONDS = 60 * 60 * 24 * 90;

/**
 * 暗号論的に安全なランダムIDを生成
 * セキュリティ: Math.random()は予測可能なため使用しない
 */
...
export async function saveTrace(trace: Record<string, unknown>): Promise<void> {
  ...
  try {
    const traceId = generateSecureId(TRACE_PREFIX);
    await kv.set(traceId, trace, { ex: TRACE_EXPIRY_SECONDS });

    // Add to sorted set for ordering (score = timestamp)
    await kv.zadd('traces:index', { score: Date.now(), member: traceId });

    // Keep only last MAX_GLOBAL_TRACES traces
    const count = await kv.zcard('traces:index');
    if (count > MAX_GLOBAL_TRACES) {
      const toRemove = await kv.zrange('traces:index', 0, count - MAX_GLOBAL_TRACES - 1);
...
export async function saveWorkspaceTrace(
  ...
  try {
    ...
    await kv.set(traceKey, { ...trace, workspace_id: workspaceId }, { ex: WORKSPACE_TRACE_EXPIRY_SECONDS });
    ...
    // Keep only last MAX_WORKSPACE_TRACES traces per workspace
    const count = await kv.zcard(getWorkspaceKey(workspaceId, 'traces:index'));
    if (count > MAX_WORKSPACE_TRACES) {
      const toRemove = await kv.zrange(getWorkspaceKey(workspaceId, 'traces:index'), 0, count - MAX_WORKSPACE_TRACES - 1);
      if (toRemove.length > 0) {
...
export async function saveFeedback(feedback: TraceFeedback): Promise<void> {
  ...
  try {
    ...
    await kv.set(feedbackKey, feedback, { ex: FEEDBACK_EXPIRY_SECONDS });
  ...
  try {
    ...
    for (const key of traceKeys) {
      ...
      if (trace && (trace.requestId === traceId || (key as string).includes(traceId))) {
        ...
        await kv.set(key as string, updatedTrace, { ex: WORKSPACE_TRACE_EXPIRY_SECONDS });
