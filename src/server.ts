...
export async function build(options?: { enableAuth?: boolean; enableRateLimit?: boolean }) {
  ...
  // Input validation middleware - sanitize check for SQL injection / XSS
  fastify.addHook('preHandler', inputValidationMiddleware);

  // Budget guard middleware - blocks requests when budget exceeded
  fastify.addHook('preHandler', budgetGuardMiddleware);

  // Plan guard middleware - enforces monthly trace limits per plan
  fastify.addHook('preHandler', planGuardMiddleware);
...
export async function start() {
  ...
  try {
    ...
    fastify.log.info(`FujiTrace v0.4.0`);
