...
export interface StorageAdapter {
  ...
  updateTraceEvaluations(workspaceId: string, traceId: string, evaluations: TraceEvaluations): Promise<void>;
...
export class KVStorageAdapter implements StorageAdapter {
  ...
  async updateTraceEvaluations(workspaceId: string, traceId: string, evaluations: TraceEvaluations): Promise<void> {
    const trace = await this.getTrace(workspaceId, traceId);
    if (trace) {
      const updatedTrace = { ...trace, evaluations };
      const key = `workspace:${workspaceId}:trace:${traceId}`;
      const ttlSeconds = config.maxAgeDays * 24 * 60 * 60;
      await kv.set(key, updatedTrace, { ex: ttlSeconds });
    }
...
export class PostgresStorageAdapter implements StorageAdapter {
  ...
  async updateTraceEvaluations(workspaceId: string, traceId: string, evaluations: TraceEvaluations): Promise<void> {
    const query = `
      UPDATE traces
      SET evaluations = $1::jsonb
      WHERE id = $2 AND workspace_id = $3
    `;

    await this.pool.query(query, [JSON.stringify(evaluations), traceId, workspaceId]);
  }
  ...
  private deserializeTrace(row: Record<string, unknown>): WorkspaceTraceRecord {
    return {
      ...
      evaluations: row.evaluations ? (typeof row.evaluations === 'string' ? JSON.parse(row.evaluations) : row.evaluations as TraceEvaluations) : undefined
    };
  }
}
